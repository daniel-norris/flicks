    ...
    <div class="container mx-auto mt-8 flex justify-center">
        <div class="w-2/3 space-y-4">
            ...
            @foreach ($search['results'] as $result)
                @isset($result['title'])
                <div class="flex bg-gray-600 rounded shadow-md hover:opacity-75 text-gray-300">
                    <img class="w-32 rounded-l" src="{{ $imgBaseUrl . $result['poster_path'] }}" alt="">
                    <div class="w-1/4 p-4">
                        <h2 class="font-bold">{{ $result['title'] }}</h2>
                        <p class="font-bold text-blue-500">{{ $result['vote_average'] }}<span class="font-thin"> / 10.0</span></p>
